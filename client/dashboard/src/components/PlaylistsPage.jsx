...
function PlaylistsPage() {
  ...
  useEffect(() => {
    ...
    const fetchPlaylistData = async () => {
      try {
        setLoading(true);
        const res = await fetch(`/godspeed/playlists/${playlistId}`);
        if (!res.ok) {
          if (res.status === 404) {
        ...
        const data = await res.json();
        if (!data || !data.id) {
          throw new Error(`Invalid playlist data for ID: ${playlistId}`);
        }
  ...
  const handleCreatePlaylist = async () => {
    ...
    try {
      ...
      for (const track of newPlaylistTracks) {
        ...
        if (!trackRes.ok) throw new Error('Failed to add track to playlist');
      }
  ...
  const handleUpdatePlaylist = async () => {
    ...
    try {
      ...
      setPlaylists(playlists.map(p => p.id === editingPlaylist.id ? { ...p, ...editingPlaylist } : p));
  ...
  const handleAddTrackToPlaylist = async (playlistId, trackId) => {
    try {
      ...
      setPlaylists(playlists.map(p => p.id === playlistId ? { ...p, track_count: p.track_count + 1 } : p));
    } catch (error) {
  ...
  const handleTrackRemove = useCallback(async (trackId) => {
    try {
      ...
      setPlaylists(playlists.map(p => p.id === selectedPlaylist.id ? { ...p, track_count: p.track_count - 1 } : p));
    } catch (error) {
                        ...
                        >
                          <div>
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleAddTrackToPlaylist(selectedPlaylist?.id || null, track.id);
                                handleAddTrackToNewPlaylist(track);
                              }}
                              disabled={newPlaylistTracks.some(t => t.id === track.id) || (selectedPlaylist && tracks.some(t => t.id === track.id))}
                            >
                              <FaPlus />
                            </button>
                          </div>
                    ...
                    playlists.map(playlist => (
                      <div key={playlist.id} className="pls-card">
                        <div
                          ...
                          onClick={() => navigate(`/playlists?playlist=${playlist.id}`)}
                        >
